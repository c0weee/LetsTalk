    ...
    getListByGroupId: async (data) => {
        try {
            ...
            const result = await pool.request()
                ...
                .query('SELECT t.id AS id, t.title AS title, u.id AS userId, u.username AS createdBy, COUNT(p.id) AS [count], COALESCE(MAX(p.updatedAt), t.createdAt) AS lastReplied FROM [thread] t INNER JOIN [user] u ON t.createdBy = u.id LEFT JOIN [post] p ON t.id = p.threadId WHERE t.groupId = @groupId AND (t.title LIKE @search OR t.title IS NULL AND @search IS NULL) GROUP BY t.id, t.title, u.id, u.username, t.createdAt ORDER BY lastReplied DESC OFFSET @size * (@page - 1) ROWS FETCH NEXT @size ROWS ONLY;');
